...
player_qbr_ratingsCRY <- player_stats %>%
  left_join(CPStats, by = c("Name", "Number")) %>%
  mutate(
    passes_per90 = round((passes / Minutes) * 90, 1),
    completion_100 = pmin(pmax(((completion_pct - 0) / (92.5 - 0)) * 100, 0), 100),
    value_100      = pmin(pmax(((total_value - (-3)) / (3 - (-3))) * 100, 0), 100),
    impact_100     = pmin(pmax(((`Total Impact` - (-1)) / (1 - (-1))) * 100, 0), 100),
    QBR_100 = round(
      (completion_100 * 0.34) +
        (value_100 * 0.30) +
        (impact_100 * 0.36),
...
CPQBRPlot1<-ggplot(player_qbr_ratingsCRY, aes(x=passes_per90, y=QBR_100, image="palace_logo.svg"))+
...
CPQBRPlot2<-CPQBRPlot1+xlim(0,70)+ylim(0,100)+geom_text_repel(aes(label=Name), fontface="bold", max.overlaps=20, box.padding=.5)+
  annotate(geom="text", x=2, y=50, fontface="bold", 
           label="Average QBR", color="blue", size=3.5)+
  annotate(geom="text", x=1, y=75, fontface="bold", 
           label="Good QBR", color="darkgreen",size=3.5)+
  annotate(geom="text", x=1, y=25, fontface="bold", 
           label="Poor QBR", color="red",size=3.5)
CPQBRPlot2

#histogram of xP values#
xPHistogram<-ggplot(data=PassModel_final%>%filter(Foot=="W"))+geom_histogram(aes(x=xP_xgboost), binwidth = .02, fill="steelblue",color="black")+
  labs(title="Distribution of Expected Pass Values", subtitle="n=3166, Collected from 2025-26 EPL Season", x="Expected Pass Value", y=("Count"))+
  theme_bw()+
...
#insert player data NEW GAME#
ASTData <- PassModelAST %>%
  dummy_cols(select_columns = c("Direction", "Area", "Body Pt", "Foot", "Touches",
                                "Thru", "R Air", "S Air", "H Air", "Cross"),
             remove_selected_columns = TRUE,
             remove_first_dummy = FALSE) %>%
...
missing_cols <- setdiff(colnames(X), colnames(ASTData))
ASTData[missing_cols] <- 0
ASTData <- ASTData %>% select(all_of(colnames(X)))

ASTData_matrix <- as.matrix(ASTData)

PassModelAST$xP_xgboost <- predict(Accuracy_xg, ASTData_matrix)
PassModelAST$value <- PassModelAST$Accurate - PassModelAST$xP_xgboost

player_stats_AST <- PassModelAST %>%
  group_by(Name, Number) %>%
  summarise(
    passes = n(),
    completed = sum(Accurate),
...
view(player_stats_AST)

player_qbr_ratings_AST <- player_stats_AST %>%
  left_join(ASTStats, by = c("Name", "Number")) %>%
  mutate(
    passes_per90 = round((passes / Minutes) * 90, 1),
    completion_100 = pmin(pmax(((completion_pct - 0) / (92.5 - 0)) * 100, 0), 100),
    value_100      = pmin(pmax(((total_value - (-3)) / (3 - (-3))) * 100, 0), 100),
...
View(player_qbr_ratings_AST)

download.file(
  "https://a.espncdn.com/i/teamlogos/soccer/500/362.png",
  destfile = "villa_logo.png",
  mode = "wb"
)
  
ASTQBRPlot1<-ggplot(player_qbr_ratings_AST, aes(x=passes_per90, y=QBR_100, image="villa_logo.png"))+
  geom_image(size=.08)+labs(title= "QBR Rating and Per 90 Involvement", subtitle = "Aston Villa vs. Sunderland MW33", x="Passes Attemped Per 90 Minutes", y="Quarterback Rating")+
  theme_bw()+theme(plot.title=element_text(face="bold", hjust=.5, size=14, family=""), 
                   plot.subtitle = element_text(hjust=.5))
ASTQBRPlot2<-ASTQBRPlot1+xlim(0,70)+ylim(0,100)+geom_text_repel(aes(label=Name), fontface="bold", max.overlaps=20, box.padding=.5)+
  annotate(geom="text", x=2, y=50, fontface="bold", 
           label="Average QBR", color="blue", size=3.5)+
  annotate(geom="text", x=1, y=75, fontface="bold", 
           label="Good QBR", color="darkgreen",size=3.5)+
...
#create master set#
player_qbr_ratings <- bind_rows(
  player_stats %>% left_join(CPStats, by = c("Name", "Number")) %>% mutate(Team = "CRY"),
  player_stats_AST %>% left_join(ASTStats, by = c("Name", "Number")) %>% mutate(Team = "AST")
) %>%
  mutate(
    passes_per90 = round((passes / Minutes) * 90, 1),
    completion_100 = pmin(pmax(((completion_pct - 0) / (92.5 - 0)) * 100, 0), 100),
  ...
  arrange(desc(QBR_100)) %>%
  select(Team, Name, Number, passes, passes_per90, completed, completion_pct,
         total_value, `Total Impact`, QBR_100)
